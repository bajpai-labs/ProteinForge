...
export const Route = createFileRoute("/contact")({
  head: () =>
    pageSocialHead({
      title: `Contact ProteinForge: Schedule an Antibody Design Consultation | ${PRODUCT_NAME}`,
      ...
      ogDescription: `Schedule a 30-minute intro call or request a project quote for your antibody or protein engineering program.`,
    }),
      ...
      <section className="border-b border-border bg-[var(--parchment)]">
        <div className="mx-auto max-w-7xl px-6 py-20">
          ...
          <p className="mt-5 max-w-2xl text-lg text-muted-foreground">
            ...
            whether a biologics pilot fits your timeline.
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto grid max-w-7xl gap-12 px-6 py-20 lg:grid-cols-[1.4fr_1fr]">
          ...
          {sent ? (
            ...
            <form onSubmit={onSubmit} className="space-y-5">
              ...
              <div className="grid gap-5 sm:grid-cols-2">
                ...
                <SelectField label="Project type" name="project_type" options={["Antibody discovery", "Protein design", "Affinity maturation", "Developability optimization", "Bispecific engineering", "Custom / not sure yet"]} />
              </div>
              <SelectField label="Timeline" name="timeline" options={["ASAP (within 4 weeks)", "Next quarter", "Exploring options"]} />
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider text-muted-foreground">Tell us about your project</label>
        ...
        <aside className="space-y-6">
          <div className="rounded-2xl border border-border bg-foreground p-8 text-background">
            ...
            <a href={`mailto:${CONTACT_EMAIL}?subject=${encodeURIComponent(`${PRODUCT_NAME} consultation`)}`} className="mt-5 inline-flex items-center justify-center rounded-md bg-accent px-5 py-2.5 text-sm font-semibold text-accent-foreground hover:opacity-90">
              Email to schedule
            </a>
          </div>
